...
Despite the potential for DUET to be used by researchers and model developers to improve 3D fuel modeling in forested landscapes, no standardized tool yet exists for managing the inputs and outputs of DUET simulations to help parameterize realistic, place-based 3D fire behavior simulations.\newline
...
Because the sizes of DUET output arrays are limited by DUET's capabilities, there are no meaningful differences in performance of the package across spatial domain sizes that \texttt{duet-tools} is likely to process.\newline
...
This method is best suited for situations where fuel data are limited or when the underlying distribution of calibration data does not match the distribution of the simulated DUET data.
...
This could provide an alternative to the SB40 fuel data, which, while widely used for physics-based fire modeling, may not reflect real-world fuel parameter values, especially in disturbed or managed areas.
