...
A common workflow will begin by creating a \texttt{DuetRun} object using \texttt{import_duet()}, which derives DUET data dimensions from the \texttt{duet.in} input file.
If the input file is not present, but the data dimensions are known, the user can alternatively specify \texttt{import_duet_manual()}.
...
A simple example of this process is shown in Listing 1 below.
...
In the above example, the fuel loading parameter is calibrated using separately defined targets for each fuel type.
